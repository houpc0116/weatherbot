...
const rain = require('./proc/probability.js');   //自製JavaScript
const temperature = require('./proc/temperature.js');   //自製JavaScript
  ...
  if(event.message.type == 'text') {            //判斷是文字格式的
    ...
    request("http://www.cwb.gov.tw/V7/observe/24real/Data/"+get_code+".htm", function (error, response, body) {
      if (!error && response.statusCode == 200) {
        ...
        rain.requestpost(msg, function(res) {
          //console.log(typeof res);
          var data = JSON.parse(JSON.stringify(res));
          //console.log(data.probability);
          msg_info += "降雨機率:"+data.probability+", ";
          temperature.requestpost1(msg, function(res) {
            var data = JSON.parse(JSON.stringify(res));
            msg_info += "體感溫度:"+data.temperature+", 紫外線:"+data.uvi;
            //推播訊息
