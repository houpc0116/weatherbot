...
function requestpost(area, callback){
  var code ='';
  var area = area;
...
request({
    url:  'http://www.cwb.gov.tw/V7/forecast/taiwan/'+code+'.htm'
}, function(error, response, body){
  if (!error && response.statusCode == 200) {
     var $ = cheerio.load(body);
     var rain = $(".FcstBoxTable01 tbody tr td").slice(0).eq(3).text();
     var description = {probability:rain};
     //console.log(rain);
...
module.exports.requestpost = requestpost;   //輸出模組給其他JS使用